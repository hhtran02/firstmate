...
- Pi listens for `agent_settled` in `.pi/extensions/fm-primary-turnend-guard.ts`, runs once per logical agent run, and calls `pi.sendUserMessage(..., { deliverAs: "followUp" })` once when the guard returns 2.
  The same extension registers a `tool_result` boundary for `fm_watch_arm_pi`: a missing or malformed result content array becomes renderer-safe text while legacy `details` and `usage` are preserved, a string-valued legacy `details.error` forces error status, and unrelated tools are untouched.
